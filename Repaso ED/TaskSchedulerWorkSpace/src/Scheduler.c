...
struct Node *Scheduler_new(void)
{
  ...
  struct Node *newScheduler = (struct Node *)malloc(sizeof(struct Node));
 ...
 */
bool Scheduler_enqueue(struct Node **p_p_last, const struct Task *p_task) // **p_p_last es un puntero a un puntero
{
  struct Node *aux = (struct Node *)malloc(sizeof(struct Node)); // NO ES LO MISMO EL TASK QUE EL NODE RECUERDA!!!
